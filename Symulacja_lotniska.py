"""
=============================================================================
SYMULACJA KRYZYSOWA: EFEKT KASKADOWY, GROUND STOP I METRYKI ZAAWANSOWANE
=============================================================================
"""
...
def get_delay_multiplier(airport_code, airports_res):
    #mnoznik opoznienia na podstawie wykorzystania oraz dlugosci kolejki oczekujacych

    res = airports_res[airport_code]
    # wskaźnik obciążenia: (samoloty przy bramkach + samoloty w kolejce) / liczba bramek
    util = (res.count + len(res.queue)) / res.capacity

    if util > 2.0:
        return 4.0   # ogromna kolejka
    elif util > 1.5:
        return 2.0   # poważne zatory
    elif util > 1.0:
        return 1.3   # początek opóźnień
    return 1.0
...
def flight_process(env, f_id, src, dst, airports_res):
    ...
    #nie puszczamy nowych lotow do zamknietych lotnisk
    while not AIRPORT_STATUS[src]['is_open'] or not AIRPORT_STATUS[dst]['is_open']:
        yield env.timeout(3)

    # rezerwacje bramki na start i kaskadowe opoznienia
    with airports_res[src].request() as req:
        yield req
        multiplier = get_delay_multiplier(src, airports_res)
        yield env.timeout(random.uniform(10, 20) * multiplier)
    ...
    # przekierowanie samolotu w powietrzu
    ...
    with airports_res[dst].request() as req:
        yield req
        multiplier_dst = get_delay_multiplier(dst, airports_res)
        yield env.timeout(random.uniform(10, 15) * multiplier_dst)
...
print("1. Symulowanie ruchu ...")
env.run(until=SIM_DURATION_MIN)

#metryki
def analyze_results():
    print("\n" + "="*70)
    print(" Zebrane metryki:")
    print("="*70)
    print(f"{'Lotnisko':<20} | {'Wszystkie Loty':<15} | {'Uziemione':<10} | {'Śr. Opóźnienie':<15}")
    print("-" * 70)

    stats = {}
    for airport in AIRPORTS:
        flights_here = [f for f in FLIGHT_LOGS if f["src"] == airport]

        wait_times = []
        stranded_count = 0
        for f in flights_here:
            if f["start"] != float('inf'):
                wait_times.append(f["start"] - f["created"])
            else:
                stranded_count += 1

        stats[airport] = {
            "total_flights": len(flights_here),
            "avg_wait_min": np.mean(wait_times) if wait_times else 0,
            "max_wait_min": np.max(wait_times) if wait_times else 0,
            "stranded": stranded_count,
        }

        name_abrv = (AIRPORTS[airport]["name"][:17] + '..') if len(AIRPORTS[airport]["name"]) > 19 else AIRPORTS[airport]["name"]
        print(f"{name_abrv:<20} | {stats[airport]['total_flights']:<15} | {stats[airport]['stranded']:<10} | {stats[airport]['avg_wait_min']:>8.1f} min")

    print("="*70 + "\n")
    return stats

# zrzut metryk do konsoli
analyze_results()
...
def animate(frame):
    ...
    ax_map.set_title(f"Wizualizacja symulacji — {h:02d}:{m:02d}", color="#ffffff", fontsize=12, weight="bold")
    ...
    y_pos = 0.8
    ax_txt.text(0.02, 0.9, "Komunikaty kontroli ruchu:", color="#8888aa", weight="bold", fontsize=10)
    for alert in reversed(live_alerts):
        color = "#ff3333" if "AKTYWACJA" in alert or "BLOKADA" in alert else "#00ff66"
...
ani.save('symulacja_zaawansowana.gif', writer='pillow', fps=14)
plt.close()
print("Wizualizacja wygenerowana (symulacja_zaawansowana.gif)")
